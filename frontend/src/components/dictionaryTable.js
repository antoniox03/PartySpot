...
export default function DictionaryTable({ dictionary, dictionaryartist }) {
              ...
              .map((row, index) => (
                ...
                <TableRow hover role="checkbox" tabIndex={-1} key={index}>
                  {columns.map((column) => {
                    ...
                    const correspondingArtist = dictionaryartist[rowIndex];
                    return (
                      <TableCell key={column.id} align={column.align}>
                        {column.id === 'artist' && correspondingArtist ? correspondingArtist.value : value}
                      </TableCell>
                    );
                  })}
                </TableRow>
